...
import { normalizeCitySlug } from "@/utils/citySearch";
...
const sortByIdentifierOrder = (items, identifiers = []) => {
  const orderMap = new Map();
  identifiers.forEach((identifier, index) => {
    orderMap.set(toText(identifier, "").toLowerCase(), index);
  });

  return [...items].sort((a, b) => {
    const aId = toText(a?.id, "").toLowerCase();
    const aSlug = toText(a?.slug, "").toLowerCase();
    const bId = toText(b?.id, "").toLowerCase();
    const bSlug = toText(b?.slug, "").toLowerCase();
...
export const getPropertiesByIds = async (identifiers = [], limit = 4) => {
  ...
  if (BACKEND_BASE_URL) {
    try {
      ...
      const endpoint = buildBackendUrl(BACKEND_LIST_PATH, params.toString());
      if (endpoint) {
        const response = await fetch(endpoint, {
          cache: "no-store",
          headers: { Accept: "application/json" },
        ...
        if (response.ok) {
          const payload = await response.json();
          const items = normalizePropertyCollection(extractListItems(payload));
          if (items.length) {
            return sortByIdentifierOrder(items, normalizedIds).slice(0, safeLimit);
          }
        }
      }
    } catch {
  ...
  const lookup = new Set(normalizedIds.map((id) => id.toLowerCase()));
  const localMatches = LOCAL_DATASET.filter((item) => {
    const itemId = toText(item?.id, "").toLowerCase();
    const itemSlug = toText(item?.slug, "").toLowerCase();
    return lookup.has(itemId) || lookup.has(itemSlug);
  });

  return sortByIdentifierOrder(localMatches, normalizedIds).slice(0, safeLimit);
};
